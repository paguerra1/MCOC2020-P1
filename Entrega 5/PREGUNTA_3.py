# -*- coding: utf-8 -*-
"""
Created on Sun Sep  6 21:14:40 2020
...
sol_eulerint = eulerint(satelite,z0,t,5000)
t3=perf_counter()
...
plt.figure(1)
plt.subplot(3,1,1)
plt.title("Posición del Sentinel(Nsub=5000)")
plt.ylabel("X(t) [KM]")
...
plt.tight_layout()
plt.savefig("Preg3_Posición_Nsub=5000")
...
plt.title(f" Distancias (Nsub=5000): $\\delta_{{odeint}} = {delta_odeint[-1]/1000:.1f}$ [Km] , $\\delta_{{eulerint}} = {delta_euler[-1]/1000:.1f}$ [Km]")
plt.ylabel(" $\\delta$ [KM]")
plt.xlabel("Tiempo[hora]")
plt.tight_layout()
plt.legend()
plt.savefig("Preg3_Deriva_EulerintVSOdeint_Nsub=5000")


plt.figure(3)
plt.plot(hora,delta_euler/1000,label="Eulerint", c="green")
plt.title(f" Distancia entre posición real y predicha (Nsub=5000 )$\\delta_{{max}} = {delta_euler[-1]/1000:.1f}$ [Km]")
plt.ylabel(" $\\delta$ [KM]")
plt.xlabel("Tiempo[hora]")
plt.tight_layout()
plt.legend()
plt.savefig("Preg3_Deriva_Eulerint_Nsub=5000")
...
plt.tight_layout()
plt.legend()
plt.savefig("Preg3_Deriva_Odeint")
plt.show()
